...
-(id)initWithSize:(CGSize)size {
    if (self = [super initWithSize:size]) {
        ...
        SpaceCatNode *spaceCat = [SpaceCatNode spaceCatAtPosition:CGPointMake
                                  (machine.position.x, machine.position.y-2)];
        ...
        GroundNode *ground = [GroundNode groundWithSize:CGSizeMake
                              (self.frame.size.width, 22)];
...
- (void) addSpaceDog{
    ...
    float y = self.frame.size.height + spaceDog.self.size.height;
...
-(void) update:(NSTimeInterval)currentTime{
    ...
    if(self.totalGameTime > 480){ //480
        self.addEnemyTimeInterval = 0.50;
        self.minSpeed = -160;
    } else if(self.totalGameTime > 240){ //240
        self.addEnemyTimeInterval = 0.65;
        self.minSpeed = -150;
    } else if(self.totalGameTime > 120){ //120
        self.addEnemyTimeInterval = 0.75;
        self.minSpeed = -125;
    } else if(self.totalGameTime > 30){ //30
...
-(void)didBeginContact:(SKPhysicsContact *)contact{
    ...
    if(firstBody.categoryBitMask == CollisionCategoryEnemy &&
        ...
        [self addPoints:ConstPointsPerHit];
    ...
    [self createDebrisAtPoint:contact.contactPoint];
}
...
-(void) createDebrisAtPoint:(CGPoint)position{
    NSInteger numberOfPeices = [Util randomWithMin:5 max:20];
